...
namespace beethoven3
{
    class BackgroundItemShop : ItemShop
    {
        ...
        private Texture2D backgroundItemShopBackground;

        MouseState mouseStatePrevious;
        private Item selectedItem;

        public BackgroundItemShop(ItemManager itemManager,ScoreManager scoreManager, ReportManager reportManager)
        ...
        public override void LoadContent(ContentManager cm)
        {
            ...
            backgroundItemShopBackground = cm.Load<Texture2D>(@"backgroundItem\backgroundItemBackground");
            setLocationItems();
        }
        ...
        {
            itemManager.removeMyBackgroundItem(item);
        ...
        public void setLocationItems()
        {
            ...
            else
            {
                ...
                for (i = 3; i < backgroundItems.Count; i++)
                {
        ...
        public override void Update(GameTime gameTime, Rectangle rightHandPosition)
        {
            ...
            int i;
            //아이템 rect
            List<Rectangle> rectBackgroundItems = getRectBackgroundItem();

            //아이템/
            List<Item> shopBackgroundItems = getShopBackgroundItem();
            ...
            if (!getSellOrWearOne() && !getBuyOne() && !getNoGold() && !getHandInItem())
            {
                ...
                for (i = 0; i < rectBackgroundItems.Count; i++)
                {
                    ...
                    if (mouseRect.Intersects(rectBackgroundItems[i]) || rightHandPosition.Intersects(rectBackgroundItems[i]))
                    {
                        ...
                        Game1.GetCenterOfButton(rectBackgroundItems[i]);
                        ...
                        if ((mouseStateCurrent.LeftButton == ButtonState.Pressed && mouseStatePrevious.LeftButton == ButtonState.Released) || (Game1.finalClick && !Game1.pastClick))
                        {
                            ...
                            selectedItem = shopBackgroundItems[i];

                            //이미 산거이면 true
                            if (haveOne(shopBackgroundItems[i]))
                            {
                                //장착 메시지 or 팔기 메시지 박스 띄우기 
                                setSellOrWearOne(true);
                                //반복 없애기
                                i = rectBackgroundItems.Count;
                            }
                            else
                            {
                                //구입 메시지 박스 띄우기 
                ...
                if (count == rectBackgroundItems.Count)
                {
        ...
        public override void Draw(SpriteBatch spriteBatch, int width, int height)
        {
            ...
            //배경
            spriteBatch.Draw(backgroundItemShopBackground, new Rectangle(0, 0, 1024, 769), Color.White);
            ...
            for (i = 0; i < backgroundItems.Count; i++)
            {
                ...
                spriteBatch.Draw(ItemBackground, rectBackgroundItems[i], Color.White);
                //Item I already have
                //if (haveOne(backgroundItems[i]))
                //{

                //    spriteBatch.Draw(backgroundItems[i].ItemSprite.Texture, new Vector2(rectBackgroundItems[i].Center.X, rectBackgroundItems[i].Center.Y), new Rectangle(0, 0, backgroundItems[i].ItemSprite.Texture.Width, backgroundItems[i].ItemSprite.Texture.Height), Color.White, 0f, new Vector2(backgroundItems[i].ItemSprite.Texture.Width / 2, backgroundItems[i].ItemSprite.Texture.Height / 2), 0.7f, SpriteEffects.None, 1.0f);

                //}

                //else
                //{
                //    spriteBatch.Draw(backgroundItems[i].ItemSprite.Texture, new Vector2(rectBackgroundItems[i].Center.X, rectBackgroundItems[i].Center.Y), new Rectangle(0, 0, backgroundItems[i].ItemSprite.Texture.Width, backgroundItems[i].ItemSprite.Texture.Height), Color.Black, 0f, new Vector2(backgroundItems[i].ItemSprite.Texture.Width / 2, backgroundItems[i].ItemSprite.Texture.Height / 2), 0.7f, SpriteEffects.None, 1.0f);
                ...
                if (haveOne(backgroundItems[i]))
                {
                    spriteBatch.Draw(backgroundItems[i].Thumnail, rectBackgroundItems[i], Color.White);
                ...
                {
                    spriteBatch.Draw(backgroundItems[i].Thumnail, rectBackgroundItems[i], Color.Black);
                }
                ...
                spriteBatch.Draw(Game1.menuGold, new Vector2(rectBackgroundItems[i].X + 5, rectBackgroundItems[i].Y + rectBackgroundItems[i].Height - rectBackgroundItems[i].Height / 6), new Rectangle(0, 0, 25, 31), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);

                //아이템 가격 표시 
                spriteBatch.DrawString(Game1.georgia, backgroundItems[i].GetCost().ToString(), new Vector2(rectBackgroundItems[i].X + 40, rectBackgroundItems[i].Y + rectBackgroundItems[i].Height - rectBackgroundItems[i].Height / 4), Color.White);


            }

            if (itemManager.getBackgroundIndex() != -1)
            {

                wearItemLocation = new Vector2(rectBackgroundItems[itemManager.getBackgroundIndex()].X, rectBackgroundItems[itemManager.getBackgroundIndex()].Y);
