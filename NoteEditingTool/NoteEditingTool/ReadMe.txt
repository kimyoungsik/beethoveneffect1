================================================================================
    MFC 라이브러리 : NoteEditingTool 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 NoteEditingTool 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 NoteEditingTool 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

NoteEditingTool.vcxproj
...
NoteEditingTool.vcxproj.filters
...
NoteEditingTool.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CNoteEditingToolApp 응용 프로그램
    클래스를 선언합니다.

NoteEditingTool.cpp
    응용 프로그램 클래스 CNoteEditingToolApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
res\NoteEditingTool.ico
    응용 프로그램의 아이콘으로 사용되는 아이콘 파일입니다. 이 아이콘은
    주 리소스 파일인 NoteEditingTool.rc에 의해 포함됩니다.

res\NoteEditingTool.rc2
...
NoteEditingToolDoc.h, NoteEditingToolDoc.cpp - 문서
    이 파일에는 CNoteEditingToolDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CNoteEditingToolDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      mnf
        파일 형식 ID:        NoteEditingTool.Document
        주 프레임 캡션:  NoteEditingTool
        문서 형식 이름:       NoteEditingTool
        필터 이름:         NoteEditingTool Files (*.mnf)
        파일의 새 약식 이름: NoteEditingTool
        파일 형식의 긴 이름: NoteEditingTool.Document

NoteEditingToolView.h, NoteEditingToolView.cpp - 문서 뷰
    이 파일에는 CNoteEditingToolView 클래스가 들어 있습니다.
    CNoteEditingToolView 개체는 CNoteEditingToolDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 NoteEditingTool.pch와
